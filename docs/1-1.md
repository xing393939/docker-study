...
* RBAC：
  ...
  * 查询用户的角色：kubectl get clusterrolebinding -o jsonpath='{range .items\[?(@.subjects\[0].name=="default")]}\[{.roleRef}]{end}'
...
* Istio在每个Pod里注入一个Envoy容器(高性能网络代理)：部署一个envoy-initializer控制器，死循环来注入Envoy容器
* CRD，自定义API对象network的例子：github.com/xing393939/samplecrd-code
  ...
  * PS：Informer使用Reflector包，它是通过ListAndWatch机制获取并监听API对象变化的客户端封装
